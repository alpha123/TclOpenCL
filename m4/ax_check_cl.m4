...
AC_ARG_WITH(opencl, AC_HELP_STRING([--with-opencl], [directory containing OpenCL library]), with_opencllib="${withval}")
AC_CACHE_CHECK([for OpenCL library], [ax_cv_check_cl_libcl],
[# Check if --with-opencl was specified
if test x"$with_opencllib" != x; then
   CL_LIBS="-L\"$with_opencllib\" $CL_LIBS"
fi
